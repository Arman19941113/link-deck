// SavedLink card showing a saved link summary with right-click menu actions.
...
/** Shows one saved link and provides open and right-click menu actions. */
export function LinkCard({
  ...
  function handleContextMenu(): void {
    linkActionRef.current?.focus({ preventScroll: true })
    onHomeFocusCapture?.()
  }
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger asChild onContextMenu={handleContextMenu}>
        ...
        >
          <a
            ...
            className={cn(
              'flex min-w-0 flex-1 cursor-pointer items-center rounded-md text-left outline-none transition-colors duration-300 ease-app-hover group-hover:bg-secondary/25 motion-reduce:transition-none',
      ...
      <ContextMenuContent onClick={stopMenuEvent} onKeyDown={stopMenuEvent} onPointerDown={stopMenuEvent}>
        <ContextMenuGroup>
          <ContextMenuItem onSelect={() => onEditLink(link)}>
