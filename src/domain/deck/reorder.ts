...
export function moveLinkWithReorderedSiblings(
  links: SavedLink[],
  activeId: string,
  ...
  const movedLink: SavedLink = {
    ...activeLink,
    categoryId: targetCategoryId,
    updatedAt: now,
  }
  const replacements = new Map<string, SavedLink>()

  if (sourceCategoryId === targetCategoryId) {
    const orderedLinks = sortByOrder(links.filter(link => link.categoryId === sourceCategoryId && link.id !== activeId))

    for (const link of withSequentialOrder(insertAt(orderedLinks, movedLink, targetIndex))) {
      replacements.set(link.id, link)
    }
  } else {
    const sourceLinks = sortByOrder(links.filter(link => link.categoryId === sourceCategoryId && link.id !== activeId))
...
function withSequentialOrder(links: SavedLink[]): SavedLink[] {
