...
function parseHttpUrl(value: string): URL | null {
  ...
  if (
    SCHEME_LIKE_PREFIX.test(trimmed) &&
    !EXPLICIT_HTTP_SCHEME.test(trimmed) &&
    !isHostPortInput(trimmed)
  ) {
    return null;
  }
...
/** Normalizes user input into an http/https URL, returning null for invalid input. */
...
export function getUrlOrigin(value: string): string | null {
  return parseHttpUrl(value)?.origin ?? null;
}

/** Builds the link favicon URL locally so navigation data is not sent to third-party services. */
