...
function hasCategoryChanged(left: Category | undefined, right: Category): boolean {
  return !left || left.name !== right.name || left.order !== right.order || left.updatedAt !== right.updatedAt
}
...
export function useDeckStore(): DeckStore {
  ...
  const upsertLink = useCallback(
    async (input: LinkInput): Promise<Link> => {
      ...
      const nextIcon = savedIconFile ? createFileIcon(savedIconFile) : (input.icon ?? existingLink?.icon ?? EMPTY_ICON)
  ...
  const addCategory = useCallback(
    async (nameInput: string): Promise<Category> => {
      ...
      const category: Category = {
        ...
        name,
        order: latestCategories.reduce((maxOrder, categoryItem) => Math.max(maxOrder, categoryItem.order), 0) + 1,
        createdAt: now,
        updatedAt: now,
      }
  ...
  const deleteCategory = useCallback(
    async (categoryId: string, options?: DeleteCategoryOptions): Promise<void> => {
      ...
      if (categoryLinks.length > 0 && !options) {
        const optionError = createDisplayError('This category contains links. Choose whether to move or delete them.')
  ...
  const saveCategoryDraft = useCallback(
    async (draft: CategoryDraft): Promise<void> => {
      ...
      const now = new Date().toISOString()
      const latestCategoryMap = new Map(latestCategories.map(category => [category.id, category]))
      ...
      const deletedCategories = latestCategories.filter(category => !draftCategoryIds.has(category.id))
      ...
      for (const category of deletedCategories) {
        ...
        if (deletePlan === undefined) {
          const planError = createDisplayError('This category contains links. Choose whether to move or delete them.')
