...
async function hasLegacySiteRecords(
  db: IDBPDatabase<LinkDeckDb>,
  linkCount: number,
): Promise<boolean> {
...
function getDeckTimestamp(
  ...
  const timestamps = records
    .flatMap((record) =>
      type === "createdAt" ? [record.createdAt] : [record.updatedAt, record.createdAt],
    )
  ...
  return new Date(
    type === "createdAt" ? Math.min(...timestamps) : Math.max(...timestamps),
  ).toISOString();
}
...
async function ensureDefaultCategory(): Promise<void> {
  ...
  const minOrder = categories.reduce(
    (currentMinOrder, category) => Math.min(currentMinOrder, category.order),
    1,
  );
...
async function saveSettingsPatch(
  patch: Partial<Pick<SettingsRecord, "interfaceSize" | "sortMode">>,
): Promise<void> {
  const db = await dbPromise;
